...
public class MarkdownTest
{
	...
	void testDamage()
	{
		...
		assertEquals(
				"цель получает (<dice-roller formula=\"19к12\"/>) дробящего урона и <dice-roller formula=\"6к6\"/> психического урона",
	...
	void testDice()
	{
		...
		MarkdownUtil.Markdown markdown = MarkdownUtil.Markdown.builder()
				.mark("\\{@dice\\s[0-9]{0,}(к|d)[0-9]+(\\s?\\+\\s?\\d+)?\\}")
				.pattern("[0-9]{0,}(к|d)[0-9]+(\\s?\\+\\s?\\d+)?")
				.template("<dice-roller formula=\"%s\"/>")
				.build();

		assertEquals(
				"бросить <dice-roller formula=\"к4\"/> кости и <dice-roller formula=\"2к6\"/>",
				markdown.convert(description),
				"Markdown dice error"
		);
	}
	...
	void testAttack()
	{
		...
		assertEquals(
				"<em>Рукопашная атака оружием:</em> @{hit 3} к попаданию, досягаемость 10 фт., одна цель",
				markdown.convert(description),
	...
	void testHit()
	{
		...
		assertEquals(
				"<p>@{atk mw} <dice-roller label=\"Бросок атаки\" formula=\"к20 + 3\">+3</dice-roller> к попаданию, досягаемость 10 фт., одна цель.",
				markdown.convert(description),
				"Markdown hit error"
		);
	}
	...
	void testSpell()
	{
		...
		MarkdownUtil.Markdown markdown = MarkdownUtil.Markdown.builder()
				.mark("@\\{spell\\s[A-Za-zА-Яа-я_\\']+\\}")
				.pattern("(?<=spell\\s)[A-Za-zА-Яа-я_\\']+")
				.template("<detail-tooltip type=\"spell\"><a href=\"/spells/%1$s\">щит [%1$s]</a></detail-tooltip>")
				.build();

		assertEquals(
				"<detail-tooltip type=\"spell\"><a href=\"/spells/shield\">щит [shield]</a></detail-tooltip> ",
				markdown.convert(description),
