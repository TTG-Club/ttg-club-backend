...
public class InitiativeCombatService {
	...
	public void nextTurn(InitiativeTracker tracker, List<InitiativeParticipant> participants) {
		...
		InitiativeParticipant next = result.getParticipant();
		if (result.isWrapped()) {
	...
	public void prevTurn(InitiativeTracker tracker, List<InitiativeParticipant> participants) {
		...
		tracker.setCurrentParticipantId(result.getParticipant().getId());
	}
	...
	/** Результат шага по порядку хода: найденный участник и признак перехода через край (смена раунда). */
	private static final class TurnStep {
		private final InitiativeParticipant participant;
		private final boolean wrapped;

		private TurnStep(InitiativeParticipant participant, boolean wrapped) {
			this.participant = participant;
			this.wrapped = wrapped;
		}

		private InitiativeParticipant getParticipant() {
			return participant;
		}
